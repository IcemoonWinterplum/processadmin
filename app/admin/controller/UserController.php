...
class UserController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $userEmail = trim($this->request->param('user_nickname'));
        ...
        $users = Db::name('user')
            ...
            ->where(function (Query $query) use ($userLogin, $userEmail) {
                ...
                if ($userEmail) {
                    $query->where('user_nickname', 'like', "%$userEmail%");
                }
            })
            ->order("id DESC")
            ->paginate(10);
        $users->appends(['user_login' => $userLogin, 'user_nickname' => $userEmail]);
        // 获取分页显示
        $page = $users->render();
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            if (!empty($_POST['role_id']) && is_array($_POST['role_id'])) {
                ...
                unset($_POST['role_id']);
                $result = true;//$result = $this->validate($this->request->param(), 'User');
                if ($result !== true) {
                    $this->error($result);
                } else {
    ...
    public function editPost()
    {
        if ($this->request->isPost()) {
            if (!empty($_POST['role_id']) && is_array($_POST['role_id'])) {
                ...
                unset($_POST['role_id']);
                $result = true;//$this->validate($this->request->param(), 'User.edit');
    ...
    public function leave()
    {
        $id = $this->request->param('id', 0, 'intval');
        if (!empty($id)) {
            $result = Db::name('user')->where(["id" => $id, "user_type" => 1])->setField('user_status', '3');
            if ($result !== false) {
                $this->success("管理员停用成功！", url("user/index"));
            } else {
                $this->error('管理员停用失败！');
